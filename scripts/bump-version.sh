...
# Bump the shared product version and the two store build counters, in one place.
#
...
# are deliberately *not* shared: they have separate histories, each store requires its own to strictly
# increase per upload, and bumping one is what triggers that platform's release workflow.
#
# So this does the two things that always go together at release time and are easy to half-do by hand:
# move the version by one minor step (1.43.0 -> 1.44.0) everywhere, and advance each counter by one.
# The failure mode it exists to prevent is silent — a shell left on the previous number still builds,
# still uploads, and only looks wrong in the store listing.
#
...
#
# Usage:
#   ./scripts/bump-version.sh              # highest of the three, minor + 1, patch 0
#   ./scripts/bump-version.sh 2.0.0        # bump to an explicit version instead
...
# which runs from CI once the version bump is on main.
...
printf '%-22s %-24s %-8s -> %s\n' \
  "package.json" "version" "$root_version" "$new_version" \
  "build.gradle" "versionName" "$android_version" "$new_version" \
  "build.gradle" "versionCode" "$android_code" "$new_android_code" \
  "project.pbxproj" "MARKETING_VERSION" "$ios_version" "$new_version" \
...
perl -0pi -e "s/(\"version\"\s*:\s*)\"[^\"]*\"/\${1}\"$new_version\"/" package.json

perl -pi -e "s/(versionCode\s+)\d+/\${1}$new_android_code/" "$GRADLE"
perl -pi -e "s/(versionName\s+)\"[^\"]*\"/\${1}\"$new_version\"/" "$GRADLE"
...
perl -pi -e "s/(MARKETING_VERSION = )[^;]+;/\${1}$new_version;/g" "$PBX"
...
check "package.json version" "$(node -p "require('./package.json').version")" "$new_version"
check "$GRADLE versionName" "$(read_or_die "$GRADLE" 'versionName\s+"([^"]+)"' versionName)" "$new_version"
check "$GRADLE versionCode" "$(read_or_die "$GRADLE" 'versionCode\s+(\d+)' versionCode)" "$new_android_code"
check "$PBX MARKETING_VERSION" "$(read_or_die "$PBX" 'MARKETING_VERSION = ([^;]+);' MARKETING_VERSION)" "$new_version"
...
for key in "MARKETING_VERSION = $new_version;" "CURRENT_PROJECT_VERSION = $new_ios_build;"; do
...
echo
echo "Bumped to $new_version (android build $new_android_code, ios build $new_ios_build)."

if ! grep -q "^## $new_version\([^0-9.]\|$\)" CHANGELOG.md; then
  echo
  echo "CHANGELOG.md has no '## $new_version' entry yet — scripts/release.sh falls back to"
  echo "--generate-notes without one, and that fallback is what /news would then show."
fi
