...
export function OrientationFilterText(props: {
  ...
  const convertedTypes = options.map((o) =>
    t(`profile.orientation.${toKey(o)}`, INVERTED_ORIENTATION_CHOICES[o] ?? o),
  )
...
export function OrientationFilter(props: {
  ...
  const t = useT()
  const selected = ((filters as any).orientation as string[] | undefined) ?? []
  const hasExtendedSelected =
    selected.some((v) => EXTRA_ORIENTATIONS.includes(v as any)) ||
    (profile?.orientation &&
      profile?.orientation.some((v) => EXTRA_ORIENTATIONS.includes(v as any)))
  const [showAll, setShowAll] = useState(hasExtendedSelected)
  // console.log({showAll, hasExtendedSelected, profile})

  const visibleChoices = Object.fromEntries(
    Object.entries(ORIENTATION_CHOICES).filter(
      ([, v]) => showAll || DEFAULT_ORIENTATIONS.includes(v as any) || selected.includes(v),
    ),
  )
