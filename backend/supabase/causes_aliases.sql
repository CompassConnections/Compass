-- Names that used to be their own `causes` option and were merged away.
--
...
--
-- Mirrors the `causes_translations` layout rather than one polymorphic table keyed by table name, so
-- the foreign key and its ON DELETE CASCADE are real.
CREATE TABLE IF NOT EXISTS causes_aliases
(
...
-- Row Level Security
ALTER TABLE causes_aliases
    ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "public read" ON causes_aliases;
CREATE POLICY "public read" ON causes_aliases
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_causes_aliases_lower
    ON causes_aliases (lower(alias));
