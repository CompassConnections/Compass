...
export function MessagesContent(props: {currentUser: User}) {
  ...
  // The store-review ask used to live here, on a six-second timer. It almost never fired: this list
  // is somewhere people pass through on the way into a thread, so the tap that followed unmounted the
  // page and cancelled it. It now hangs off leaving a conversation instead — see
  // `useReviewPromptOnConversationExit` in `web/hooks/use-review-prompt.ts`, which still lands the
  // card here, just on the way back out rather than the way in.
