 ...
 *
 * Bottom sheet under `sm`, popover anchored to the button above it. The sheet is portalled to the
 * document body rather than left in place: its usual home is the profile page's sticky header, whose
 ...
 * bottom nav. The desktop popover stays inline, where `absolute` positioning against the trigger is
            ...
            >
              <PopoverPanel
                ...
                className={clsx(
                  ...
                  isMobile
                    ? // pb keeps the last row clear of the phone's own gesture bar.
                      'fixed inset-x-0 bottom-0 z-[60] rounded-t-2xl border-t p-3 pb-[calc(0.75rem+var(--bnh))] shadow-2xl'
                    : 'absolute right-0 z-30 mt-2 w-80 rounded-xl border p-2 shadow-lg',
                )}
              >
                {/* Grabber: the one cue that says "this is a sheet, you can dismiss it". */}
                {isMobile && <div className="bg-ink-300 mx-auto mb-3 h-1 w-10 rounded-full" />}
                ...
                {(canNativeShare || xShareUrl || linkedinUrl) && (
